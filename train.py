...
def train(model, train_loader, epoch, optimizer, criterion, use_gpu=False):
...
def save_model(model, model_folder):
    model_file_name = f'model.pth'
    model_file = os.path.join(model_folder, model_file_name)
    torch.save(model, model_file)
    return model_file


def train_model(model, data_loader, epochs, lr, model_path, use_gpu, criterion,
                regularization=0.0):
    ...
    for epoch in range(1, epochs + 1):
        train_loss = train(model, train_loader, epoch, optimizer, criterion, use_gpu)
        val_loss, val_score = evaluate(model, val_loader, criterion, use_gpu)
    ...
    print(f'Best [Val {metric}]:{best_val_score:>7.4f} | Loss: {best_val_loss:>.4f}')
